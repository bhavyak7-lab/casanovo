...
def setup_model(
    ...
    ----------
    model : str | None
        May be a file system path, a URL pointing to a .ckpt file,
        None, or a selector. If `model` is a URL the weights will be
        downloaded and cached from `model`. If `model` is `None` the
        weights from the latest matching official release will be used
        (downloaded and cached). If `model` is a selector, the weights
        from the latest matching release to the selector will be used
